class Translation < ActiveRecord::Base
	...
	scope :finished, -> {where("output_text IS NOT NULL").order("updated_at DESC")}
	scope :unfinished, -> {where("output_text IS NULL").order("created_at DESC")}
